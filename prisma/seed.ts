...
import { defaultPlannedShipDate } from "../lib/business-days";
...
// 実データ: 要求元の「デモ機マスタ.csv」より（[備品ID, 備品名, カテゴリー, シリアル№]）
const UNIT_ROWS: [string, string, string, string][] = [
  ...
  ["NK150-01", "NK150デモ機①", "NK150・200 デモ機", "259D218"],
  ["NK150-02", "NK150デモ機②", "NK150・200 デモ機", "215D198"],
  ["NK150-03", "NK150デモ機③", "NK150・200 デモ機", "2411D391"],
  ["NK150-04", "NK150デモ機④", "NK150・200 デモ機", "2411D392"],
  ["NK150-05", "NK150デモ機⑤", "NK150・200 デモ機", "257D473"],
  ["NK200-01", "NK200デモ機①", "NK150・200 デモ機", "224E101"],
  ["NK200-02", "NK200デモ機②", "NK150・200 デモ機", "224E105"],
  ["NK228-150-01", "NK228-150", "NK228 デモ機", ""],
  ["NK228-200-01", "NK228-200", "NK228 デモ機", "229G071"],
  ["NK228-200-02", "NK228-200", "NK228 デモ機", "229G066"],
  ...
  ["LEX3130BK-02", "カネカ有機EL② LEX-3130BK　4000K（無期限/無償貸出）※不点灯使用不可", "有機EL デモ機", ""],
...
async function main() {
  ...
  const units = await Promise.all(
    UNIT_ROWS.map(([assetNo, name, categoryName, serial]) =>
  ...
  await prisma.reservation.createMany({
    data: [
      // 過去・返却済み
      {
        ...
        customerName: "佐藤様",
        shipToName: "みなと設計事務所 3F 設計室",
        shipToContact: "佐藤様",
        shipToPhone: "03-1234-5678",
        shipToPostal: "105-0001",
        shipToAddress: "東京都港区虎ノ門1-2-3 みなとビル3F",
        startDate: day(-30),
        plannedShipDate: defaultPlannedShipDate(day(-30)),
        endDate: day(-23),
        shipDate: day(-30),
        desiredArrivalDate: day(-29),
        desiredArrivalTime: "AM",
      ...
      // 返却遅延（PICKED_UP かつ endDate 過去）
      {
        ...
        customerName: "田中様",
        startDate: day(-10),
        plannedShipDate: defaultPlannedShipDate(day(-10)),
        endDate: day(-3),
        shipDate: day(-10),
        desiredArrivalDate: day(-9),
      ...
      // 現在貸出中
      {
        ...
        shipToName: "幕張メッセ 国際展示場 ホール5 搬入口",
        shipToContact: "国際イベント企画 現場事務局 高橋",
        shipToPhone: "043-296-0001",
        shipToPostal: "261-0023",
        shipToAddress: "千葉県千葉市美浜区中瀬2-1",
        startDate: day(-2),
        plannedShipDate: defaultPlannedShipDate(day(-2)),
        endDate: day(4),
        shipDate: day(-2),
        desiredArrivalDate: day(-1),
        desiredArrivalTime: "14〜16時",
        pickupLocationId: honsha.id,
        returnLocationId: honsha.id,
        status: "PICKED_UP",
        pickedUpAt: day(-2),
      ...
      // 本日出庫予定（確定）
      {
        ...
        customerName: "鈴木様",
        startDate: day(0),
        plannedShipDate: defaultPlannedShipDate(day(0)),
        endDate: day(6),
        pickupLocationId: honsha.id,
        returnLocationId: honsha.id,
        status: "CONFIRMED",
      ...
      // 未来・確定
      {
        ...
        startDate: day(5),
        plannedShipDate: defaultPlannedShipDate(day(5)),
        endDate: day(12),
      ...
      // 未来・確定
      {
        ...
        customerName: "山本様",
        startDate: day(9),
        plannedShipDate: defaultPlannedShipDate(day(9)),
        endDate: day(16),
        pickupLocationId: amagasaki.id,
        returnLocationId: honsha.id,
        status: "CONFIRMED",
      ...
      // 未来・確定（別機）
      {
        ...
        startDate: day(3),
        plannedShipDate: defaultPlannedShipDate(day(3)),
        endDate: day(8),
        pickupLocationId: amagasaki.id,
        returnLocationId: amagasaki.id,
        status: "CONFIRMED",
